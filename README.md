...
- 🔧 工具调用（查询材料、价格、收款、算账、催债）
- 💬 流式输出（逐字显示，打字机效果）
- 📊 Token 统计（每次对话 + 总计）
- 🔍 详细调试输出（SelectionStrategy、TerminationStrategy、工具调用）
- 💰 财务精确计算（成本、利润、账单明细）
- 💀 暴力催债系统（威胁、施压、强制收款）
...
程序运行时会显示详细的调试信息，帮助理解 AgentGroupChat 的运行流程：

1. **SelectionStrategy 调试**：
   - 显示上一个发言者
   - 显示 LLM 选择结果
   - 显示下一个发言者

2. **TerminationStrategy 调试**：
   - 显示关键词检测结果
   - 显示 LLM 判断结果
   - 显示是否终止对话
...
🔧 [工具调用调试] 张三 调用了 1 个工具
   📌 工具名称: get_equipment_materials
   📌 工具参数: {'equipment_name': '飞剑'}
   ✅ 工具执行成功
   📤 返回结果: 制作【飞剑】需要材料：玄铁 x3, 寒冰石 x2, 灵木 x1
...
**剧情流程：**
```
玩家选装备 → 张三打造（分5步，李四协助） → 麻子算账（详细账单） 
→ 李四收款 → 玩家不给钱 → 李四请肖斩天出马 
→ 肖斩天威胁 → 肖斩天施压（拍桌子） → 肖斩天强制收款 
→ 玩家继续不给 → 肖斩天继续催债（循环直到给钱）
→ 玩家给钱 → 肖斩天放狠话 → 李四交钱给张三 → 张三满意 → 结束
```

**催债机制：**
- 肖斩天会持续催债，不会放弃
- 即使超过对话轮数限制，只要检测到催债关键词就会继续
- 只有玩家支付后才会结束对话
...
1. 李四收款失败后，会说："看来得请肖斩天大哥出马了"
2. 肖斩天出场，先威胁玩家
3. 然后施压（拍桌子、跺脚等）
4. 最后强制收款，问玩家给不给钱
5. 如果玩家还不给，肖斩天会继续催债（循环）
6. 直到玩家给钱为止

**技术实现：**
- TerminationStrategy 检测到"不给钱"、"犹豫"等关键词时，强制不终止对话
- SelectionStrategy 会让肖斩天连续发言，持续催债
- 即使超过 max_rounds（50轮），只要在催债就会继续

### Q3: 如何查看调试信息？

程序运行时会自动显示详细的调试信息：

- 🔍 **SelectionStrategy 调试**：显示如何选择下一个发言者
- 🔍 **TerminationStrategy 调试**：显示如何判断是否终止
- 🔧 **工具调用调试**：显示工具名称、参数、执行结果

这些信息帮助你理解 AgentGroupChat 的运行流程。
...
修改 `main_groupchat.py` 中的 `max_rounds` 参数：

```python
chat = AgentGroupChat(agents=agents, llm=llm, max_rounds=50)  # 默认 50 轮
```

注意：即使超过 max_rounds，如果检测到催债关键词，对话仍会继续。

### Q7: 为什么肖斩天不说脏话？

为了符合 Azure OpenAI 的内容政策，我们移除了所有粗俗和暴力词汇：
- ❌ 不使用：老子、他妈的、打人、揍你、狗东西等
- ✅ 使用：严厉警告、施压、威慑力等

肖斩天通过严肃的语气和动作描述（拍桌子、跺脚）来展示威慑力，而不是暴力。
