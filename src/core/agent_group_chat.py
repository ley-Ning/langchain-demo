...
6. 当肖斩天催债时：
   - 如果工具返回"玩家还在犹豫"或"不给钱" → 下一个必须还是肖斩天（继续催债）
   - 如果工具返回"玩家被震慑住了"或"支付" → 下一个必须是李四（把钱交给张三）
7. 当李四把钱交给张三 → 下一个必须是张三（说几句结束语）

特殊规则：
- 麻子只在算账时发言，其他时候不发言
- 肖斩天在催债过程中可以连续发言多次，直到收到钱为止
- 打造过程中主要是张三和李四轮流
...
只有当满足以下条件时，才回复"是"：
- 玩家已经支付了灵石（明确看到"玩家已支付"、"玩家被震慑住了，乖乖支付"、"交易完成"、"支付成功"等关键词）
- 并且张三已经说了结束语（如"好好好"、"满意"、"下次再来"等）

其它任意情况都必须回复"否"，包括：
- 还在打造装备
- 还在报价
- 玩家取消了支付（看到"取消了支付"、"取消支付"、"已取消支付"等关键词）- 这种情况必须回复"否"！
- 玩家还在犹豫不给钱（看到"犹豫"、"不给钱"、"想跑"等关键词）- 这种情况必须回复"否"！
- 肖斩天正在催债（看到"肖斩天"、"催债"、"威胁"等关键词）- 这种情况必须回复"否"！
- 张三让李四去收款但李四还没有发起支付
- 李四刚刚劝说玩家但玩家还没有重新支付
- 李四或张三刚刚说完话，但对话还没有真正结束

特别重要：
1. 如果看到"取消"、"已取消"、"不给钱"、"犹豫"等关键词，必须回复"否"
2. 只有明确看到"玩家被震慑住了，乖乖支付"或"玩家已支付"，并且张三说了结束语，才能回复"是"
3. 如果不确定，就回复"否"
